  ...
  return (
    <section className="py-16 container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {steps.map((step) => (
          <Card key={step.number} className="story-card border-t-4 hover:translate-y-[-5px] transition-transform" style={{ borderTopColor: step.color.replace('bg-', '') === 'bg-rasti-blue' ? '#0091D7' : step.color.replace('bg-', '') === 'bg-rasti-red' ? '#E42528' : step.color.replace('bg-', '') === 'bg-rasti-yellow' ? '#FDC300' : '#49B675' }}>
